# HydroPro Tracker — Changelog Archive (v1.0.0 – v2.9.0)

**Frozen historical record.** Covers first release through v2.9.0 — August 11–14, 2026. Current release history lives in `CHANGELOG.md`.

Kept in the repo for provenance, and deliberately *not* in project knowledge. The app was restructured in v3.0.0 (navigation) and again across v3.9.0–v3.16.0 (tiles, scheduling, inventory, page layout), so most specifics below describe structures that no longer exist. The recurring lessons from this period are distilled at the top of the current `CHANGELOG.md` rather than being re-read from here.

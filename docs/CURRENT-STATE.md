...
## What shipped Aug 19–21, 2026 (v3.17.0–3.33.0)
...
- **UX-OPEN-02 — Backdrop-click cascade:** tapping the OO modal backdrop closes the parent Log sheet too (React tree-based event bubbling). Pre-existing, not a regression. Fix is a one-liner `stopPropagation` on the OO backdrop, same as My Presets already has. *Needs Rob's real-device confirmation first, then Claude can fix alone.*
...
**Working sequence (updated Aug 21):** UX-OPEN-02 backdrop fix (one-liner, pending Rob's device confirm) → UX-OPEN-01 Phase 2 (Settings redesign) → server as source of truth (`ARCH-OPEN-04`) → clinic-side build (`ARCH-OPEN-02`).

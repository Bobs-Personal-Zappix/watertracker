...
- **v3.36.2 patch, same day** — the "Log" button rename in v3.36.1 wasn't the real fix: the button had a transparent background the whole time (`.wt-btn-primary`'s `background:var(--deep)` — `--deep` only ever existed in `.wt-root`'s scope, never merged into the global `:root` tokens, so it resolves to nothing for `BackfillSheet`/`OO`'s portaled content — same bug class as the `--paper` sheet-background fix in v3.34.0, just not caught for this specific property). White text on a transparent background = invisible but still clickable, exactly matching Rob's report. Fixed comprehensively: both `BackfillSheet` and `OO` (which shares the identical portal structure and had the *identical* unreported bug on its own "Save preset" button and `.wt-modal` background) now locally redefine `--deep`/`--line`/`--paper` alongside the existing `--muted` override. Also bumped the Log button's `marginTop` to match Log It!'s Log button exactly, per Rob's explicit ask.
...
- **Token adoption sweep partial.** v3.35.0/v3.36.0 converted Log It!'s 8 tiles, My Plan's 8 chips, and category colors app-wide. Every other literal `#fff` card/surface background and `rgba(0,0,0,…)` shadow elsewhere in the CSS (My Plan card backgrounds themselves, Stats stat cards, sheets/modals, etc.) is still untouched — candidate for `v3.37.0` (sheet standardization + polish).
...
**Working sequence (updated Aug 21, post-v3.36.0):** `UX-OPEN-02`, the Log It! tile restructure, and this round of real-device visual refinements are all done. Next up, **v3.37.0 = sheet standardization + polish**. After that: `UX-OPEN-01` Phase 2 (Settings redesign) → server as source of truth (`ARCH-OPEN-04`) → clinic-side build (`ARCH-OPEN-02`).

...
import {
  ...
  Zap,
    ...
    var cS = "water-tracker-data",
        ...
        ES = {
            ...
            settings: {
                ...
                supplements: [],
                rx: [],
                __preMigrationSupplementsBackup: null,
                treatments: [],
                partners: [],
                __prePartnerMigrationBackup: null,
                // v3.64.0 (D): tracks exactly which partner records the demo seeder created, so
                // "Clear demo data" can remove precisely those and nothing else. Deliberately NOT
                // a field on the partner records themselves (settings.partners' own shape is
                // unchanged, per the brief's scope discipline) — a sibling bookkeeping key instead.
                demoSeed: {
                    active: !1,
                    partnerIds: []
                },
    ...
    function BS(e) {
        return !e || "sleep" === e.type || "weight" === e.type || "supplement" === e.type || "rx" === e.type || "treatment" === e.type || "exercise" === e.type || void 0 === e.type && void 0 !== e.oz && void 0 !== e.grams && void 0 !== e.calories ? e : {
            id: e.id,
            time: e.time,
    ...
    function USTracker(e) {
        return (Array.isArray(e) ? e : []).map(e => ({
            ...
            // v3.64.0: demo-seed bookkeeping tag (D) — must be whitelisted here or it's
            // silently stripped on the next boot/migrate(), per ARCH-OPEN-05's addendum bug
            // class (these per-item normalizers aren't reached by deepMergeDefaults).
            demoSeeded: !!e.demoSeeded
        }))
    }
    ...
    function normalizePartners(e) {
        return (Array.isArray(e) ? e : []).map(e => ({
            id: e.id,
            name: e.name,
            type: "rx" === e.type ? "rx" : "treatment",
            logoDataUri: e.logoDataUri || null,
            link: e.link || null
        }))
    }

    function promotePartnersFromLegacyItems(rxItems, treatmentItems) {
        let partners = [];

        function findOrCreate(name, type, logo, link) {
            let key = name.trim().toLowerCase(),
                existing = partners.find(p => p.type === type && p.name.trim().toLowerCase() === key);
            if (existing) return existing.id;
            let id = `${Date.now()}-${Math.random().toString(36).slice(2, 7)}`;
            return partners.push({ id, name: name.trim(), type, logoDataUri: logo || null, link: link || null }), id
        }
        let rx = rxItems.map(item => item.pharmacy ? { ...item, partnerId: findOrCreate(item.pharmacy, "rx", item.partnerLogoDataUri, item.partnerLink) } : item),
            treatments = treatmentItems.map(item => item.provider ? { ...item, partnerId: findOrCreate(item.provider, "treatment", item.partnerLogoDataUri, item.partnerLink) } : item);
        return { partners, rx, treatments }
    }

    // Demo seed data (v3.64.0, brief section D). A hidden Settings action for conference-demo
    // use — populates ~30 days of realistic (imperfect) history across all trackers, two
    // partners with logos, and RX/Supplement items at varying supply levels, so every screen
    // in this session's other three sections (A/B/C) looks real in front of an audience instead
    // of empty. Every record it creates is tagged `demoSeeded:true` (log entries, rx/treatment/
    // supplement items) or tracked by id (`settings.demoSeed.partnerIds`, since partner records
    // themselves deliberately carry no new field — see that key's own comment) so "Clear demo
    // data" can remove exactly what was seeded, nothing else, without date-range guesswork.
    function demoId(prefix) {
        return `${prefix}-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`
    }

    function demoLogoDataUri(initials, bg) {
        let svg = `<svg xmlns="http://www.w3.org/2000/svg" width="120" height="120"><rect width="120" height="120" rx="24" fill="${bg}"/><text x="60" y="76" font-family="Space Grotesk, sans-serif" font-size="46" font-weight="700" fill="#ffffff" text-anchor="middle">${initials}</text></svg>`;
        return `data:image/svg+xml,${encodeURIComponent(svg)}`
    }

    function demoPickSource() {
        let roll = Math.random();
        return roll < .65 ? "manual" : roll < .9 ? "smart" : "voice"
    }

    function clearDemoSeed(data) {
        let seededPartnerIds = data.settings.demoSeed && data.settings.demoSeed.partnerIds || [],
            logs = {};
        Object.keys(data.logs || {}).forEach(dateKey => {
            let kept = (data.logs[dateKey] || []).filter(entry => !entry.demoSeeded);
            kept.length > 0 && (logs[dateKey] = kept)
        });
        return {
            ...data,
            logs,
            settings: {
                ...data.settings,
                partners: (data.settings.partners || []).filter(p => !seededPartnerIds.includes(p.id)),
                rx: (data.settings.rx || []).filter(item => !item.demoSeeded),
                treatments: (data.settings.treatments || []).filter(item => !item.demoSeeded),
                supplements: (data.settings.supplements || []).filter(item => !item.demoSeeded),
                demoSeed: { active: !1, partnerIds: [] }
            }
        }
    }

    function buildDemoSeed(data) {
        // Idempotent: re-running "Load demo data" clears any prior seed first rather than
        // accumulating duplicate partners/items/days on every click.
        let base = data.settings.demoSeed && data.settings.demoSeed.active ? clearDemoSeed(data) : data,
            today = new Date;
        today.setHours(0, 0, 0, 0);
        let farExpiry = HS(VS(today, 120)),
            nearExpiry = HS(VS(today, 5)),
            recentTaken = HS(VS(today, -1)),
            pharmacyPartner = { id: demoId("partner"), name: "Riverside Pharmacy", type: "rx", logoDataUri: demoLogoDataUri("RP", "#2E86C1"), link: "https://example.com/riverside-pharmacy" },
            clinicPartner = { id: demoId("partner"), name: "Cascade Wellness Clinic", type: "treatment", logoDataUri: demoLogoDataUri("CW", "#16A394"), link: "https://example.com/cascade-wellness" },
            rxItems = [
                { id: demoId("rx"), name: "Metformin 500mg", intervalDays: 1, lastTakenDate: recentTaken, nextDueOverride: null, trackInventory: !0, qtyRemaining: 2, expirationDate: farExpiry, pharmacy: null, refillsRemaining: 0, partnerLogoDataUri: null, partnerLink: null, partnerId: pharmacyPartner.id, demoSeeded: !0 },
                { id: demoId("rx"), name: "Lisinopril 10mg", intervalDays: 1, lastTakenDate: recentTaken, nextDueOverride: null, trackInventory: !0, qtyRemaining: 24, expirationDate: farExpiry, pharmacy: null, refillsRemaining: 3, partnerLogoDataUri: null, partnerLink: null, partnerId: pharmacyPartner.id, demoSeeded: !0 }
            ],
            treatmentItems = [
                { id: demoId("treatment"), name: "Physical Therapy", intervalDays: 7, lastTakenDate: recentTaken, nextDueOverride: null, trackInventory: !1, qtyRemaining: 0, expirationDate: null, provider: null, partnerLogoDataUri: null, partnerLink: null, partnerId: clinicPartner.id, demoSeeded: !0 }
            ],
            supplementItems = [
                { id: demoId("supplement"), name: "Vitamin D3", intervalDays: 1, lastTakenDate: recentTaken, nextDueOverride: null, trackInventory: !0, qtyRemaining: 45, expirationDate: farExpiry, pharmacy: null, refillsRemaining: null, partnerLogoDataUri: null, partnerLink: null, partnerId: null, demoSeeded: !0 },
                { id: demoId("supplement"), name: "Magnesium Glycinate", intervalDays: 1, lastTakenDate: recentTaken, nextDueOverride: null, trackInventory: !0, qtyRemaining: 12, expirationDate: nearExpiry, pharmacy: null, refillsRemaining: null, partnerLogoDataUri: null, partnerLink: null, partnerId: null, demoSeeded: !0 },
                { id: demoId("supplement"), name: "Omega-3 Fish Oil", intervalDays: 1, lastTakenDate: recentTaken, nextDueOverride: null, trackInventory: !1, qtyRemaining: 0, expirationDate: null, pharmacy: null, refillsRemaining: null, partnerLogoDataUri: null, partnerLink: null, partnerId: null, demoSeeded: !0 }
            ],
            logs = { ...base.logs },
            exerciseTypes = ["Walking", "Cycling", "Strength Training", "Yoga"];
        for (let dayOffset = 29; dayOffset >= 0; dayOffset--) {
            let day = VS(today, -dayOffset),
                dateKey = HS(day),
                entries = [...(logs[dateKey] || [])],
                startLen = entries.length;
            if (Math.random() < .85) {
                let oz = Math.round(48 + 40 * Math.random()),
                    grams = Math.round(70 + 60 * Math.random()),
                    calories = Math.round(1500 + 900 * Math.random());
                entries.push({ id: demoId("log"), time: "8:30 AM", timeMinutes: 510, label: "Breakfast", oz, grams, calories, source: demoPickSource(), sourceText: null, demoSeeded: !0 })
            }
            if (Math.random() < .8) {
                let hours = Math.round(10 * (5.5 + 3 * Math.random())) / 10,
                    wokeUp = 360 + Math.round(90 * Math.random()),
                    lightsOut = (wokeUp - Math.round(60 * hours) + 1440) % 1440;
                entries.push({ id: demoId("log"), type: "sleep", label: "Sleep", time: tO(wokeUp), timeMinutes: wokeUp, lightsOutMinutes: lightsOut, wokeUpMinutes: wokeUp, hours, source: demoPickSource(), sourceText: null, demoSeeded: !0 })
            }
            if (dayOffset % 3 == 0) {
                let value = Math.round(10 * (178 - .05 * dayOffset + (2 * Math.random() - 1))) / 10;
                entries.push({ id: demoId("log"), type: "weight", label: "Weight", time: "7:15 AM", timeMinutes: 435, value, source: demoPickSource(), sourceText: null, demoSeeded: !0 })
            }
            if (Math.random() < .5) {
                let minutes = Math.round(20 + 40 * Math.random()),
                    exerciseType = exerciseTypes[Math.floor(Math.random() * exerciseTypes.length)];
                entries.push({ id: demoId("log"), type: "exercise", label: exerciseType, exerciseType, minutes, description: "", time: "6:00 PM", timeMinutes: 1080, source: demoPickSource(), sourceText: null, demoSeeded: !0 })
            }
            if (Math.random() < .9) {
                let item = rxItems[Math.floor(Math.random() * rxItems.length)];
                entries.push({ id: demoId("log"), type: "rx", label: `Took: ${item.name}`, items: [{ name: item.name, qty: "1" }], time: "9:00 AM", timeMinutes: 540, source: demoPickSource(), sourceText: null, demoSeeded: !0 })
            }
            if (Math.random() < .85) {
                let item = supplementItems[Math.floor(Math.random() * supplementItems.length)];
                entries.push({ id: demoId("log"), type: "supplement", label: `Took: ${item.name}`, items: [{ name: item.name, qty: "1" }], time: "9:05 AM", timeMinutes: 545, source: demoPickSource(), sourceText: null, demoSeeded: !0 })
            }
            dayOffset % 7 == 0 && entries.push({ id: demoId("log"), type: "treatment", label: "Took: Physical Therapy", items: ["Physical Therapy"], time: "2:00 PM", timeMinutes: 840, source: "manual", sourceText: null, demoSeeded: !0 }),
            entries.length > startLen && (logs[dateKey] = entries)
        }
        return {
            ...base,
            logs,
            settings: {
                ...base.settings,
                partners: [...base.settings.partners, pharmacyPartner, clinicPartner],
                rx: [...base.settings.rx, ...rxItems],
                treatments: [...base.settings.treatments, ...treatmentItems],
                supplements: [...base.settings.supplements, ...supplementItems],
                demoSeed: { active: !0, partnerIds: [pharmacyPartner.id, clinicPartner.id] }
            }
        }
    }

    function migrateSettingsShape(e) {
        let alreadySplit = e && Array.isArray(e.rx),
            oldCombined = e && Array.isArray(e.supplements) ? e.supplements : [],
            rxSource = alreadySplit ? e.rx : oldCombined,
            supplementsSource = alreadySplit ? e && e.supplements : [],
            preMigrationBackup = alreadySplit ? e && e.__preMigrationSupplementsBackup || null : oldCombined.length > 0 ? oldCombined : null,
            normalizedRx = USTracker(rxSource),
            normalizedTreatments = normalizeTreatments(e && e.treatments),
            partnersAlreadyMigrated = e && Array.isArray(e.partners),
            partnerResult = partnersAlreadyMigrated ? { partners: normalizePartners(e.partners), rx: normalizedRx, treatments: normalizedTreatments } : promotePartnersFromLegacyItems(normalizedRx, normalizedTreatments),
            prePartnerMigrationBackup = partnersAlreadyMigrated ? e && e.__prePartnerMigrationBackup || null : (normalizedRx.some(i => i.partnerLogoDataUri || i.pharmacy) || normalizedTreatments.some(i => i.partnerLogoDataUri || i.provider)) ? { rx: normalizedRx, treatments: normalizedTreatments } : null;
        return {
            ...e,
            presets: FS(e),
            rx: partnerResult.rx,
            supplements: USTracker(supplementsSource),
            __preMigrationSupplementsBackup: preMigrationBackup,
            treatments: partnerResult.treatments,
            partners: partnerResult.partners,
            __prePartnerMigrationBackup: prePartnerMigrationBackup
        }
    }
    ...
    function imageOnlyTile({
        ...
        return React.default.createElement("div", {
            className: "wt-gauge-wrap wt-gauge-imageonly" + (n ? " wt-gauge-imageonly-lit" : "")
    ...
    function newSupO({
        ...
        return React.default.createElement(lO, {
            ...
            over: !1,
            overText: "",
            imageSrc: "supplements-new",
            ringColor: "var(--supplements)",
            folder: "tile-icons",
            compact: t
        })
    }
    ...
    function yO({
        ...
        return e ? React.default.createElement("div", {
            ...
            className: "wt-help-list"
        }, React.default.createElement("li", null, "Tap Log under Water, Protein, or Calories on the main screen."), React.default.createElement("li", null, "Pick a quick amount, or enter a custom one and adjust the time if you're catching up on something from earlier."), React.default.createElement("li", null, "Sleep works differently — tap Log Sleep and enter Lights Out and Woke Up times; the hours are calculated for you, including overnight sessions that cross midnight. Naps use the same button and add to the day's total."), React.default.createElement("li", null, "Made a mistake? Tap the trash icon next to the entry in My Day's log to remove it."))), React.default.createElement("div", {
            className: "wt-help-section"
        }, React.default.createElement("p", {
            className: "wt-help-title"
    ...
    function CO({
        ...
        let isVitamin = "vitamin" === catProp,
            isRx = "rx" === catProp,
            [a, o] = (0, React.useState)(""), [i, l] = (0, React.useState)("1"), [u, s] = (0, React.useState)(!1), [c, f] = (0, React.useState)(""), [d, p] = (0, React.useState)(""), [pharm, setPharm] = (0, React.useState)(""), [refills, setRefills] = (0, React.useState)(""), [selectedPartnerId, setSelectedPartnerId] = (0, React.useState)("");
        if ((0, React.useEffect)(() => {
                e && (o(t ? t.name : ""), l(t && null != t.intervalDays ? String(t.intervalDays) : "1"), s(!(!t || !t.trackInventory)), f(t && null != t.qtyRemaining ? String(t.qtyRemaining) : ""), p(t && t.expirationDate ? t.expirationDate : ""), setPharm(t && t.pharmacy ? t.pharmacy : ""), setRefills(t && null != t.refillsRemaining ? String(t.refillsRemaining) : ""), setSelectedPartnerId(t && t.partnerId ? t.partnerId : ""))
            }, [e, t]), !e) return null;
        let m = a.trim().length > 0,
            rxPartners = partnerList.filter(p => "rx" === p.type),
        ...
        return React.default.createElement("div", {
            ...
            className: "wt-modal-header"
        }, React.default.createElement("h3", null, modalTitle), React.default.createElement("button", {
            className: "wt-icon-btn",
            onClick: n,
            "aria-label": "Close"
        }, React.default.createElement(XIcon, {
            ...
            type: "text",
            placeholder: isVitamin ? "e.g. Vitamin D, Fish Oil" : "e.g. Metformin",
            value: a,
            onChange: e => o(e.target.value)
        })), isRx && React.default.createElement("label", {
            className: "wt-field"
        }, "Partner", React.default.createElement("select", {
            value: selectedPartnerId,
            onChange: e => setSelectedPartnerId(e.target.value)
        }, React.default.createElement("option", { value: "" }, "No partner (self-managed)"), rxPartners.map(p => React.default.createElement("option", { key: p.id, value: p.id }, p.name)))), isRx && 0 === rxPartners.length && React.default.createElement("p", {
            style: {
                fontSize: 11.5,
                color: wD,
                marginTop: -8,
        ...
        }, "Add partners on My Plan's My Current RX section to link this item to one."), isRx && "" === selectedPartnerId && React.default.createElement("label", {
            className: "wt-field"
        }, "Pharmacy / where filled (optional)", React.default.createElement("input", {
            type: "text",
            ...
            value: i,
            onChange: e => l(e.target.value)
        })), !isVitamin && React.default.createElement("p", {
            style: {
                fontSize: 11.5,
                color: wD,
                marginTop: -8,
            ...
            value: c,
            onChange: e => f(e.target.value)
        })), u && isRx && React.default.createElement("label", {
            className: "wt-field"
        }, "Refills remaining", React.default.createElement("input", {
            type: "number",
            inputMode: "numeric",
            min: "0",
            placeholder: "e.g. 3",
            ...
            disabled: !m,
            onClick: () => r(a.trim(), Math.max(1, Math.round(Number(i) || 1)), u, Number(c) || 0, d || null, isRx && selectedPartnerId ? null : pharm.trim() || null, isRx && refills.length > 0 ? Math.max(0, Math.round(Number(refills) || 0)) : null, isRx ? selectedPartnerId || null : null)
        }, "Save")))
    }
    ...
    function jO({
        ...
        let [a, o] = (0, React.useState)(""), [i, l] = (0, React.useState)(""), [u, s] = (0, React.useState)(!1), [c, f] = (0, React.useState)(""), [d, p] = (0, React.useState)(""), [prov, setProv] = (0, React.useState)(""), [selectedPartnerId, setSelectedPartnerId] = (0, React.useState)("");
        if ((0, React.useEffect)(() => {
                e && (o(t ? t.name : ""), l(t && t.intervalDays ? String(t.intervalDays) : ""), s(!(!t || !t.trackInventory)), f(t && null != t.qtyRemaining ? String(t.qtyRemaining) : ""), p(t && t.expirationDate ? t.expirationDate : ""), setProv(t && t.provider ? t.provider : ""), setSelectedPartnerId(t && t.partnerId ? t.partnerId : ""))
            }, [e, t]), !e) return null;
        let m = a.trim().length > 0,
            treatmentPartners = partnerList.filter(p => "treatment" === p.type);
        return React.default.createElement("div", {
            className: "wt-backdrop wt-center",
            onClick: n
        }, React.default.createElement("div", {
            ...
            value: selectedPartnerId,
            onChange: e => setSelectedPartnerId(e.target.value)
        }, React.default.createElement("option", { value: "" }, "No partner (self-managed)"), treatmentPartners.map(p => React.default.createElement("option", { key: p.id, value: p.id }, p.name)))), 0 === treatmentPartners.length && React.default.createElement("p", {
            style: {
                fontSize: 11.5,
                color: wD,
                marginTop: -8,
        ...
        }, "Add partners on My Plan's My Health Providers section to link this item to one."), "" === selectedPartnerId && React.default.createElement("label", {
            className: "wt-field"
        }, "Ordered from / provider (optional)", React.default.createElement("input", {
            type: "text",
            ...
            disabled: !m,
            onClick: () => r(a.trim(), Math.max(0, Math.round(Number(i) || 0)), u, Number(c) || 0, d || null, selectedPartnerId ? null : prov.trim() || null, selectedPartnerId || null)
        }, "Save")))
    }
    ...
    function NO({
        ...
        onGoToSettings: o,
        targetDateLabel: tdl,
        title: sheetTitle = "Supplements & Prescriptions",
        emptyCopy = "Nothing set up yet. Add the supplements, vitamins, and medicines you take regularly in Setup, and they'll show up here to log with one tap.",
        addButtonLabel = "Add to Supplements & Prescriptions",
        ...
        return React.default.createElement("div", {
            ...
            className: "wt-sheet-header"
        }, React.default.createElement("h3", null, n ? "Edit entry" : sheetTitle), React.default.createElement("button", {
            className: "wt-icon-btn",
            onClick: r,
            "aria-label": "Close"
        }, React.default.createElement(XIcon, {
        ...
        }, emptyCopy), React.default.createElement("button", {
            className: "wt-btn-primary",
            onClick: o
    ...
    function DatePill({
        ...
        d0.setHours(0, 0, 0, 0);
        let offsetDays = Math.round((d0 - today) / 864e5),
        ...
        return React.default.createElement("div", {
            ...
            size: 18,
            className: "wt-datepill-icon"
        }), React.default.createElement("span", {
            className: "wt-datepill-date"
        }, label), React.default.createElement("span", {
            className: "wt-datepill-badge"
        }, badgeText), React.default.createElement("input", {
            type: "date",
            className: "wt-datepill-input",
            value: HS(date),
            max: HS(new Date()),
            onChange: e => {
                e.target.value && onDateChange(new Date(e.target.value + "T00:00:00"))
            },
    ...
    function FeatureTile({
        ...
        let isGold = "gold" === variant;
        return React.default.createElement("div", {
            className: "wt-feature-tile " + (isGold ? "gold" : "blue"),
            role: "button",
            tabIndex: 0,
            "aria-label": clickable ? title : `${title} — coming soon, not yet active`,
            "aria-disabled": clickable ? undefined : "true",
            onClick: clickable ? onClick : undefined
        }, React.default.createElement("div", {
            className: "wt-feature-tile-icon"
        }, isGold ? React.default.createElement("svg", {
            viewBox: "0 0 24 24",
            fill: "none",
            stroke: "currentColor",
            strokeWidth: 1.8,
            ...
            width: 22,
            height: 22
        }, React.default.createElement("rect", { x: 9.5, y: 3, width: 5, height: 8.5, rx: 2.5, fill: "currentColor", stroke: "none" }), React.default.createElement("path", { d: "M7 10.5a5 5 0 0 0 10 0" }), React.default.createElement("line", { x1: 12, y1: 15.5, x2: 12, y2: 18.5 }), React.default.createElement("line", { x1: 9, y1: 18.5, x2: 15, y2: 18.5 }), React.default.createElement("path", { d: "M4.5 9.5a7.5 7.5 0 0 0 0 3" }), React.default.createElement("path", { d: "M2.3 8a10 10 0 0 0 0 6" }), React.default.createElement("path", { d: "M19.5 9.5a7.5 7.5 0 0 1 0 3" }), React.default.createElement("path", { d: "M21.7 8a10 10 0 0 1 0 6" })) : React.default.createElement(Zap, {
            size: 22,
            color: "#fff"
        })))
    }
    ...
    function computeTrackerStats(e, t) {
        let u = e.logs[t] || [],
            ...
            A = (T && (T.date === t ? T.value : (T.value, YS(MS(T.date)))), e.settings.rx.length),
            M = new Set;
        u.forEach(e => {
            RXTypeS(e) && (e.items || []).forEach(e => M.add("string" == typeof e ? e : e.name))
        });
        let _ = 0 === A ? "Add in Setup" : `${M.size} of ${A} taken`,
            D = e.settings.rx.filter(e => {
                let n = DS(e, t);
            ...
            Q = e.settings.rx.map(QS).find(e => e) || null,
            Z = e.settings.treatments.map(QS).find(e => e) || null,
            supShow = !1 !== e.settings.showSupplements,
            supTotal = e.settings.supplements.length,
            supTaken = new Set;
        u.forEach(e => {
            NS(e) && (e.items || []).forEach(e => supTaken.add("string" == typeof e ? e : e.name))
        });
        let supStatusText = 0 === supTotal ? "Add in Setup" : `${supTaken.size} of ${supTotal} taken`,
            supDueCount = e.settings.supplements.filter(e => {
                let n = DS(e, t);
                return ("overdue" === n.state || "today" === n.state) && !supTaken.has(e.name)
            }).length,
            supOverdueCount = e.settings.supplements.filter(e => {
                let n = DS(e, t);
                return "overdue" === n.state || "today" === n.state
            }).length,
            supPct = 0 === supTotal ? 0 : 0 === supOverdueCount ? 100 : (supOverdueCount - supDueCount) / supOverdueCount * 100,
            supAlert = e.settings.supplements.map(QS).find(e => e) || null;
        return {
            u, s, c, f, d, p, m, h, g, y, v, b, w, x, E, k, S, O, P, C, j, N, T, A, M, _, D, z, I, L, R, B, $, F, U, W, H, q, V, G, X, Y, K, Q, Z,
            supShow, supTotal, supStatusText, supDueCount, supOverdueCount, supPct, supAlert, supTakenCount: supTaken.size
        }
    }
    ...
    function computeAdherenceDayBucket(data, dayDate, itemsByKind) {
        ...
        [
            ...
            if (expected <= 0) return result[label] = null, void (result.raw[label] = null);
            // `logged` is a whole-dose credit (1) when an item was actually taken that day, not
            // a fractional day-share — a real dose was taken, full credit. `expected` stays the
            // fractional daily obligation (1/intervalDays). This makes the per-day display %
            // clamp to 100% on a logged day regardless of interval (a weekly item's one real
            // dose vastly exceeds that single day's fractional 1/7 obligation, clamped down to
            // 100%) while ALSO making the period aggregate correct when these raw values are
            // summed across days: total logged doses ÷ total expected doses over the period —
            // e.g. a weekly item logged exactly once in a 7-day week correctly aggregates to
            // 1÷1=100%, not the ~14% a naive per-day-percentage average would wrongly produce.
            let logged = existedByThen.reduce((sum, i) => sum + (loggedNames[label].has(i.name) ? 1 : 0), 0);
            result[label] = Math.max(0, Math.min(100, Math.round(logged / expected * 100))), result.raw[label] = { logged, expected }
        });
        return result
    }
    ...
    // v3.65.1: severity color for a single adherence bar — Rob's real-device feedback was
    // explicit that good adherence (near 100%) needs no visual noise, but low/zero adherence is
    // exactly the signal a clinic partner needs to see "glaring." Category color (--treatment/
    // --meds/--supplements) still identifies the series via the Bar's own default fill (used for
    // the Legend swatch); this only overrides each individual bar's fill by severity.
    function adherenceSeverityColor(pct) {
        return null === pct ? "transparent" : pct >= 80 ? hS : pct >= 50 ? "#E3A83B" : bS
    }
    ...
    function MO({
        ...
        onOpenNewSupplementSheet: onOpenSup2,
        ...
        if (compact) {
            let compactTile = (label, colorVar, onClick, ring) => React.default.createElement("div", {
                ...
                plainImg = imageSrc => React.default.createElement("img", {
                    src: `tile-icons/${imageSrc}.png`,
                    className: "wt-tile-plain-img",
                    alt: ""
                }),
                compactSlot = (cond, label, tile) => cond ? tile : React.default.createElement("div", {
                    key: `${label}-off`,
                    className: "wt-tracker-col-compact-off",
                    "aria-hidden": "true"
                }),
        ...
        return React.default.createElement("div", null, React.default.createElement("div", {
        ...
        }, !hideVoiceTile && React.default.createElement(FeatureTile, {
            variant: "gold",
            title: "Voice Entry",
            ...
            className: "wt-tile-goal"
        }, 0 === z.length ? "Add in Setup" : G > 0 ? `Goal ${G} today` : "Nothing planned"), Z && React.default.createElement("div", {
    ...
    function TrackedSoFar({
        ...
        return React.default.createElement("div", {
        ...
        }, "My Day at a Glance"), 0 === rows.length ? React.default.createElement("p", {
            className: "wt-empty-note"
            ...
            className: "wt-tracked-row-chip",
            style: {
                background: e.chip,
                color: e.color
            }
        }, React.default.createElement(e.icon, {
            size: 16,
            color: e.color,
            fill: e.color
        })), React.default.createElement("div", {
            className: "wt-tracked-row-text"
        }, React.default.createElement("span", {
            className: "wt-tracked-row-label"
        }, e.label), React.default.createElement("span", {
            className: "wt-tracked-row-detail"
        }, e.detail)))))
    }
    ...
    function RxPage({
        ...
        let treatments = e.settings.treatments,
            rx = e.settings.rx,
            vitamins = e.settings.supplements,
            partners = e.settings.partners;

        function renderItem(item, kind) {
            let alert = QS(item),
                expiry = item.expirationDate ? YS(MS(item.expirationDate)) : null,
                partner = item.partnerId ? partners.find(p => p.id === item.partnerId) : null,
                source = partner ? partner.name : "treatment" === kind ? item.provider : "rx" === kind ? item.pharmacy : null,
                sourceLabel = "treatment" === kind ? "From" : "Filled at",
                logoUri = partner ? partner.logoDataUri : item.partnerLogoDataUri,
                linkUrl = partner ? partner.link : item.partnerLink,
                isPartnerCard = !!(source && logoUri);
            return React.default.createElement("div", {
                key: item.id,
            ...
            }, linkUrl ? React.default.createElement("a", {
                href: linkUrl,
                target: "_blank",
            ...
            }, `${item.qtyRemaining||0} remaining`, expiry ? ` · ${"treatment" === kind ? "expires" : "renew by"} ${expiry}` : ""), "rx" === kind && null != item.refillsRemaining && React.default.createElement("div", {
                style: {
                    fontSize: 12.5,
                    color: wD,
                    marginBottom: 4
                }
            }, `${item.refillsRemaining} refill${1===item.refillsRemaining?"":"s"} left`), alert && React.default.createElement("div", {
                style: {
                    fontSize: 12.5,
                    fontWeight: 700,
                    color: bS
        ...
        return React.default.createElement("div", null, renderSection("Treatments", treatments, "treatment", "No treatments added yet. Add one on My Plan."), renderSection("Prescriptions", rx, "rx", "No prescriptions added yet. Add one on My Plan."), renderSection("Supplements", vitamins, "vitamin", "No vitamins or supplements added yet. Add one on My Plan."))
    }
    ...
    function _O({
        ...
        let p = function(e, t) {
                ...
                a.forEach(t => {
                    ...
                    n.length > 0 && s++, n.forEach(e => {
                        PS(e) ? u += e.hours || 0 : jS(e) ? c.push({
                            date: t,
                            value: e.value
                        }) : NS(e) || RXTypeS(e) ? (e.items || []).forEach(e => {
                ...
                return {
                    ...
                    // Care team (v3.64.0): only partners actually referenced by a current
                    // Treatment/Prescription item — never an orphaned partner record with
                    // nothing linked to it. Not scoped to this summary's date range, since a
                    // partner relationship isn't a log-history fact the way averages are.
                    partners: (e.settings.partners || []).filter(partner => [...e.settings.treatments, ...e.settings.rx].some(item => item.partnerId === partner.id))
                }
            }(t, r),
    ...
    function DO({
        ...
        }, r(e.rangeStart), " – ", r(e.rangeEnd), " (", e.daysInRange, " days, logged on ", e.daysWithAnyLog, ")")), e.partners && e.partners.length > 0 && React.default.createElement("div", {
            className: "wt-doctor-share-section"
        }, React.default.createElement("h2", null, "Care Team"), React.default.createElement("div", {
            className: "wt-doctor-share-partners"
        }, e.partners.map(partner => React.default.createElement("div", {
            key: partner.id,
            className: "wt-doctor-share-partner"
        }, partner.logoDataUri ? React.default.createElement("img", {
            src: partner.logoDataUri,
            alt: ""
        }) : React.default.createElement("div", {
            className: "wt-doctor-share-partner-noimg"
        }, partner.name.charAt(0).toUpperCase()), React.default.createElement("span", {
            className: "wt-doctor-share-partner-name"
        }, partner.name))))), React.default.createElement("div", {
            className: "wt-doctor-share-section"
    ...
    function IO(e, t, n, r, a = t) {
        ...
        }, e.time), React.default.createElement("div", {
            className: "wt-log-desc-stack"
        }, React.default.createElement("span", {
        ...
        }, e.value, "lbs")))) : NS(e) || RXTypeS(e) ? React.default.createElement(React.default.Fragment, null, React.default.createElement(Pill, {
            size: 13,
            className: "wt-log-icon"
        }), React.default.createElement("span", {
            className: "wt-log-time"
        ...
        }, " · "), e.amount, e.unit))))), React.default.createElement("div", {
            className: "wt-log-actions"
    ...
    function BackfillSheet({
        ...
        let [o, i] = (0, React.useState)(t || HS(new Date)),
            ...
            [k, S] = (0, React.useState)(""), [O, P] = (0, React.useState)([]), [ORx, PRx] = (0, React.useState)([]);
        if ((0, React.useEffect)(() => {
                e && (i(t || HS(new Date)), u(""), c(""), d(""), m(""), g(""), v(""), w(""), E(""), S(""), P([]), PRx([]))
            }, [e, t]), !e) return null;
        let C = !1 !== n.settings.showWater,
            ...
            showRxFlag = !1 !== n.settings.showRx,
            D = n.settings.supplements,
            DRx = n.settings.rx,
            z = Number(l) > 0 || Number(f) > 0 || Number(h) > 0 || Number(b) > 0 || Number(x) > 0 || Number(k) > 0 || O.length > 0 || ORx.length > 0,
            I = o && z;

        function LRx(e) {
            PRx(t => t.some(t => t.name === e) ? t.filter(t => t.name !== e) : [...t, {
                name: e,
                qty: "1"
            }])
        }
        ...
        return createPortal(React.default.createElement("div", {
        ...
        })), showRxFlag && React.default.createElement(React.default.Fragment, null, React.default.createElement("p", {
            style: {
                fontSize: 11.5,
                fontWeight: 700,
                color: wD,
        ...
        }, "RX — tap everything taken"), 0 === DRx.length ? React.default.createElement("p", {
            className: "wt-empty-note",
            style: {
                marginBottom: 14
            }
        }, "Nothing set up yet. Add prescriptions in My Plan first.") : React.default.createElement(React.default.Fragment, null, React.default.createElement("div", {
            className: "wt-chip-row",
            style: {
                marginBottom: ORx.length > 0 ? 14 : 18
            }
        }, DRx.map(e => React.default.createElement("button", {
            key: e.id,
            type: "button",
            className: "wt-chip " + (ORx.some(t => t.name === e.name) ? "active" : ""),
            onClick: () => LRx(e.name)
        }, e.name))), ORx.length > 0 && React.default.createElement("div", {
            style: {
                marginBottom: 16
            }
        }, ORx.map(e => React.default.createElement("div", {
            key: e.name,
            className: "wt-qty-row"
        }, React.default.createElement("span", {
            className: "wt-qty-name"
            ...
            placeholder: "Qty (e.g. 1 or 10mg)",
            value: e.qty,
            onChange: t => RRx(e.name, t.target.value)
        ...
        }, "Supplements — tap everything taken"), 0 === D.length ? React.default.createElement("p", {
            className: "wt-empty-note",
            style: {
                marginBottom: 14
            }
        }, "Nothing set up yet. Add supplements in My Plan first.") : React.default.createElement(React.default.Fragment, null, React.default.createElement("div", {
            className: "wt-chip-row",
            style: {
    ...
    function RO({
        ...
        return React.default.createElement("div", null, React.default.createElement(TrackedSoFar, {
            ...
            className: "wt-section-label wt-section-label-lg"
        }, "My Day's log"), React.default.createElement("div", {
    ...
    function FO({
        ...
        let [n, r] = (0, React.useState)("water"), [a, o] = (0, React.useState)("week"), [i, l] = (0, React.useState)(new Date), [priorDaysOpen, setPriorDaysOpen] = (0, React.useState)(!1), [priorDaysHistoryDate, setPriorDaysHistoryDate] = (0, React.useState)(null), [priorDaysBackfillOpen, setPriorDaysBackfillOpen] = (0, React.useState)(!1), [adherencePartnerFilter, setAdherencePartnerFilter] = (0, React.useState)(null), [adherencePeriod, setAdherencePeriod] = (0, React.useState)("week"), [adherenceDate, setAdherenceDate] = (0, React.useState)(new Date), u = "combined" === n, s = "protein" === n ? "g" : "calories" === n ? "cal" : "oz", c = "protein" === n ? e.settings.goalProtein || 0 : "calories" === n ? e.settings.goalCalories || 0 : e.settings.goalOz || 0, f = "protein" === n ? "grams" : "calories" === n ? "calories" : "oz";

        function d(e) {
            r(e), "combined" === e && "day" === a && o("week")
        }
        // v3.65.1: Adherence pulled out of the metric segment control per Rob's real-device
        // feedback into its own always-visible section (like Weight/Sleep Over Time below) — it
        // gets its own period/date state so navigating it never moves the Water/Protein/Calories
        // chart above and vice versa.
        let adherenceIsCurrentPeriod = "month" === adherencePeriod ? adherenceDate.getFullYear() === (new Date).getFullYear() && adherenceDate.getMonth() === (new Date).getMonth() : qS(adherenceDate).getTime() === qS(new Date).getTime(),
            adherenceData = (0, React.useMemo)(() => {
                let today = new Date;
                today.setHours(0, 0, 0, 0);
                let days, rangeLabel;
                if ("month" === adherencePeriod) {
                    let start = GS(adherenceDate);
                    days = Array.from({ length: XS(adherenceDate) }, (v, idx) => VS(start, idx)), rangeLabel = aO(adherenceDate)
                } else {
                    let start = qS(adherenceDate);
                ...
                }, dayBuckets = days.map(day => computeAdherenceDayBucket(e, day, itemsByKind)),
                    series = dayBuckets.map((bucket, idx) => ({ label: "month" === adherencePeriod ? String(days[idx].getDate()) : fS[(days[idx].getDay() + 6) % 7], Treatments: bucket.Treatments, Prescriptions: bucket.Prescriptions, Supplements: bucket.Supplements, isFuture: bucket.isFuture })),
                    pastDays = days.filter(day => day.getTime() <= today.getTime()), asNeededCount = countAsNeededLogged(e, pastDays, itemsByKind),
                    // Period-aggregate % per category, computed by summing RAW logged/expected
                    // amounts across every past day in the period first and dividing once — NOT
                    // by averaging the per-day display percentages. A perfectly-followed
                    // every-7-days item reads 100%/0% per individual day (6 misses, 1 hit) by
                    // design (matches "shaped like the existing bar chart"), but averaging those
                    // day percentages would wrongly read it as ~14% adherent instead of the
                    // correct 100%. This is what actually drives the "glaring" callout below —
                    // a single day is too noisy a signal to call a patient out over (v3.65.1).
                    aggregates = ["Treatments", "Prescriptions", "Supplements"].map(label => {
                        let totals = dayBuckets.filter((bucket, idx) => !bucket.isFuture && bucket.raw[label]).reduce((acc, bucket) => (acc.logged += bucket.raw[label].logged, acc.expected += bucket.raw[label].expected, acc), { logged: 0, expected: 0 });
                        return { label, pct: totals.expected > 0 ? Math.round(totals.logged / totals.expected * 100) : null }
                    }).filter(agg => null !== agg.pct);
                return { series, rangeLabel, asNeededCount, aggregates }
            }, [adherencePeriod, adherenceDate, e.logs, e.settings.treatments, e.settings.rx, e.settings.supplements, adherencePartnerFilter]),
        ...
        return React.default.createElement("div", null, React.default.createElement("div", {
            ...
            className: "wt-section-label wt-section-label-lg"
        }, "Adherence Over Time"), React.default.createElement("div", {
            className: "wt-segment",
            style: { marginBottom: 8 }
        }, React.default.createElement("button", {
            className: "week" === adherencePeriod ? "active" : "",
            onClick: () => {
                setAdherencePeriod("week"), setAdherenceDate(new Date)
            }
        }, "Week"), React.default.createElement("button", {
            className: "month" === adherencePeriod ? "active" : "",
            onClick: () => {
                setAdherencePeriod("month"), setAdherenceDate(new Date)
            }
        }, "Month")), React.default.createElement("div", {
            className: "wt-range-nav"
        }, React.default.createElement("button", {
            onClick: () => setAdherenceDate("week" === adherencePeriod ? e => VS(e, -7) : e => new Date(e.getFullYear(), e.getMonth() - 1, 1)),
            "aria-label": "Previous"
        }, React.default.createElement(ChevronLeft, { size: 16 })), React.default.createElement("span", {
            className: "wt-range-label"
        }, adherenceData.rangeLabel), React.default.createElement("button", {
            onClick: () => setAdherenceDate("week" === adherencePeriod ? e => VS(e, 7) : e => new Date(e.getFullYear(), e.getMonth() + 1, 1)),
            disabled: adherenceIsCurrentPeriod,
            "aria-label": "Next"
        }, React.default.createElement(ChevronRight, { size: 16 }))), e.settings.partners.length > 0 && React.default.createElement("div", {
            className: "wt-chip-row",
        ...
        }, "self" === adherencePartnerFilter ? "Filtered to self-managed items" : `Filtered to ${(e.settings.partners.find(pt => pt.id === adherencePartnerFilter) || {}).name || "partner"}`), adherenceData.aggregates.length > 0 && React.default.createElement("div", {
            style: { marginBottom: 10 }
        }, adherenceData.aggregates.filter(agg => agg.pct < 80).sort((x, y) => x.pct - y.pct).length > 0 ? adherenceData.aggregates.filter(agg => agg.pct < 80).sort((x, y) => x.pct - y.pct).map(agg => React.default.createElement("div", {
            key: agg.label,
            style: {
                display: "flex",
                alignItems: "center",
                ...
                borderRadius: 10,
                marginBottom: 6,
                background: agg.pct < 50 ? "rgba(193,82,62,.18)" : "rgba(227,168,59,.18)",
                border: `1.5px solid ${agg.pct < 50 ? bS : "#E3A83B"}`
            }
        }, React.default.createElement(AlertCircle, { size: 16, color: agg.pct < 50 ? bS : "#E3A83B" }), React.default.createElement("span", {
            style: { fontWeight: 700, fontSize: 13, color: "var(--ink-inverse)" }
        }, `${agg.label}: ${agg.pct}% this ${adherencePeriod} — needs attention`))) : React.default.createElement("div", {
            style: {
                display: "flex",
                alignItems: "center",
                gap: 8,
            ...
            style: { fontWeight: 700, fontSize: 13, color: hS }
        }, "✓ On track — all categories at 80%+ adherence"))), React.default.createElement("p", {
            className: "wt-card-note",
            style: { marginBottom: 10 }
        ...
        }, React.default.createElement(CartesianGrid, { vertical: !1, stroke: vS }), React.default.createElement(XAxis, {
            dataKey: "label",
            tick: { fontSize: 10, fill: wS },
            interval: "month" === adherencePeriod ? Math.ceil(adherenceData.series.length / 6) : 0,
            axisLine: { stroke: vS },
            tickLine: !1
        }), React.default.createElement(YAxis, {
            domain: [0, 100],
            ...
            contentStyle: { fontSize: 12, borderRadius: 8, border: `1px solid ${vS}` }
        }), React.default.createElement(Legend, { wrapperStyle: { fontSize: 11 } }), React.default.createElement(ReferenceLine, { y: 100, stroke: dS, strokeDasharray: "3 3" }), React.default.createElement(Bar, {
            dataKey: "Treatments",
            fill: "var(--treatment)",
            radius: [3, 3, 0, 0]
        }, adherenceData.series.map((dpt, idx) => React.default.createElement(Cell, {
            key: idx,
            fill: adherenceSeverityColor(dpt.Treatments),
            opacity: dpt.isFuture ? .25 : 1
        }))), React.default.createElement(Bar, {
            ...
            key: idx,
            fill: adherenceSeverityColor(dpt.Prescriptions),
            opacity: dpt.isFuture ? .25 : 1
        }))), React.default.createElement(Bar, {
            dataKey: "Supplements",
            fill: "var(--supplements)",
            ...
            key: idx,
            fill: adherenceSeverityColor(dpt.Supplements),
            opacity: dpt.isFuture ? .25 : 1
        }))))), adherenceData.asNeededCount > 0 && React.default.createElement("p", {
            className: "wt-card-note",
            style: { margin: "8px 0 0" }
        }, `+ ${adherenceData.asNeededCount} as-needed dose${1 === adherenceData.asNeededCount ? "" : "s"} logged this period (no fixed schedule, not included above)`)), React.default.createElement("div", {
            className: "wt-divider"
        }), React.default.createElement("div", {
            className: "wt-section-label wt-section-label-lg"
    ...
    function WO({
        ...
        onEditTreatment: E,
        onDeleteTreatment: k,
        onEditSupplementNextDue,
        onAddPartner,
        onEditPartner,
        onDeletePartner
    }) {
        let [S, O] = (0, React.useState)(String(e.settings.goalOz || "")), [P, C] = (0, React.useState)(String(e.settings.goalProtein || "")), [j, N] = (0, React.useState)(String(e.settings.goalCalories || "")), [T, A] = (0, React.useState)(String(e.settings.goalSleepHours || "")), [M, _] = (0, React.useState)(!1), [D, z] = (0, React.useState)(null), [I, L] = (0, React.useState)(!1), [R, B] = (0, React.useState)(null), [$, F] = (0, React.useState)(!1), [U, W] = (0, React.useState)(null), [expandedList, setExpandedList] = (0, React.useState)(null), [openTracker, setOpenTracker] = (0, React.useState)(null), [showAddPartner, setShowAddPartner] = (0, React.useState)(!1), [editingPartner, setEditingPartner] = (0, React.useState)(null), [vitOpen, setVitOpen] = (0, React.useState)(!1), [vitInitial, setVitInitial] = (0, React.useState)(null), [partnerFormName, setPartnerFormName] = (0, React.useState)(""), [partnerFormType, setPartnerFormType] = (0, React.useState)("treatment"), [partnerFormLogo, setPartnerFormLogo] = (0, React.useState)(null), [partnerFormLink, setPartnerFormLink] = (0, React.useState)(""), [partnerLogoError, setPartnerLogoError] = (0, React.useState)("");
        (0, React.useEffect)(() => {
            showAddPartner && (setPartnerFormName(editingPartner ? editingPartner.name : ""), setPartnerFormType(editingPartner ? editingPartner.type : "treatment"), setPartnerFormLogo(editingPartner ? editingPartner.logoDataUri : null), setPartnerFormLink(editingPartner ? editingPartner.link || "" : ""), setPartnerLogoError(""))
        }, [showAddPartner, editingPartner]), (0, React.useEffect)(() => {
        ...
        let X = !1 !== e.settings.showWater,
            ...
            JRx = !1 !== e.settings.showRx,
            ee = !1 !== e.settings.showTreatments,
            te = !1 !== e.settings.showExercise,
            vitaminItems = e.settings.supplements.slice().sort((e, t) => e.name.localeCompare(t.name)),
            rxItems = e.settings.rx.slice().sort((e, t) => e.name.localeCompare(t.name)),
            vitaminPreview = vitaminItems.slice(0, 3).map(e => e.name).join(" · ") + (vitaminItems.length > 3 ? " · ..." : ""),
            rxPreview = rxItems.slice(0, 3).map(e => e.name).join(" · ") + (rxItems.length > 3 ? " · ..." : ""),
            treatmentPreview = [...e.settings.treatments].sort((e, t) => e.name.localeCompare(t.name)).slice(0, 3).map(e => e.name).join(" · ") + (e.settings.treatments.length > 3 ? " · ..." : "");
        return React.default.createElement("div", {
            style: {
                overflowX: "hidden"
            }
            ...
            className: "wt-plan-section-label"
        }, "My Health Providers"), 0 === e.settings.partners.length ? React.default.createElement("div", {
            className: "wt-regimen-card wt-partner-empty"
        }, React.default.createElement(Users, {
            size: 26,
            style: { color: "var(--teal)", marginBottom: 8 }
        }), React.default.createElement("p", {
            className: "wt-partner-card-name",
            style: { marginBottom: 6 }
        }, "No health providers yet"), React.default.createElement("p", {
            className: "wt-empty-note",
            style: { margin: "0 0 14px" }
        }, "Add the clinic, pharmacy, or provider behind a Treatment or Prescription. Their logo and link travel with every item they're linked to, and with anything you share with your doctor."), React.default.createElement("button", {
            type: "button",
            className: "wt-btn-primary",
            style: { width: "100%" },
            onClick: () => {
                setEditingPartner(null), setShowAddPartner(!0)
            }
        }, React.default.createElement(Plus, { size: 18 }), " Add Partner")) : React.default.createElement(React.default.Fragment, null, e.settings.partners.map(partner => {
            let items = [...e.settings.treatments, ...e.settings.rx].filter(i => i.partnerId === partner.id);
            return React.default.createElement("div", {
                key: partner.id,
                className: "wt-regimen-card clinic wt-partner-card",
                onClick: () => {
                    setEditingPartner(partner), setShowAddPartner(!0)
                }
            }, React.default.createElement("div", {
                className: "wt-partner-card-head"
            }, partner.logoDataUri ? React.default.createElement("img", {
                src: partner.logoDataUri,
                alt: "",
                className: "wt-partner-card-logo"
            }) : React.default.createElement("div", {
                className: "wt-partner-card-logo wt-partner-card-logo-placeholder"
            }, React.default.createElement(Users, { size: 24 })), React.default.createElement("div", {
                style: { flex: 1, minWidth: 0 }
            }, React.default.createElement("div", {
                className: "wt-partner-card-name"
            }, partner.name), React.default.createElement("div", {
                className: "wt-partner-card-meta"
            }, "rx" === partner.type ? "Pharmacy" : "Treatment provider", " · ", items.length, " item", 1 === items.length ? "" : "s"))), React.default.createElement("div", {
                style: {
                    display: "flex",
                    gap: 10,
                    marginTop: 12,
            ...
            }, partner.link ? React.default.createElement("a", {
                href: partner.link,
                target: "_blank",
                rel: "noopener noreferrer",
                className: "wt-btn-secondary",
                ...
                onClick: e => e.stopPropagation()
            }, React.default.createElement(ExternalLink, { size: 14 }), "Visit") : React.default.createElement("span", { style: { flex: 1 } }), React.default.createElement("button", {
                className: "wt-btn-text wt-btn-text-danger",
                onClick: e => {
                    e.stopPropagation(), onDeletePartner(partner.id)
                }
            }, "Delete"))
            )
        }), React.default.createElement("button", {
            ...
            size: 20
        }), " Add Partner")), React.default.createElement("div", {
            className: "wt-plan-section-label"
        }, "My Current RX"), React.default.createElement(RegimenSummaryCard, {
            icon: Pill,
            iconBg: "var(--supplements-chip)",
            iconColor: "var(--supplements)",
            label: "Supplements",
            count: vitaminItems.length,
            preview: vitaminPreview,
            expanded: "vitamins" === expandedList,
            onClick: () => setExpandedList("vitamins" === expandedList ? null : "vitamins")
        }), "vitamins" === expandedList && React.default.createElement(PlanSheet, {
            title: "Supplements",
            onClose: () => setExpandedList(null)
        }, 0 === vitaminItems.length && React.default.createElement("p", {
            className: "wt-empty-note",
            style: {
                marginBottom: 10
            }
        }, "Nothing added yet. Add the vitamins and supplements you take daily so they show up as one-tap options in Log It!. These are daily items — no scheduling needed."), vitaminItems.map(item => React.default.createElement("div", {
            key: item.id,
            className: "wt-preset-row"
        }, React.default.createElement("span", {
            className: "wt-preset-name"
        }, item.name), QS(item) && React.default.createElement("span", {
            style: {
                fontSize: 11,
                fontWeight: 700,
                color: bS,
        ...
        }, QS(item)), React.default.createElement("button", {
            className: "wt-icon-btn",
            onClick: () => {
                setVitInitial(item), setVitOpen(!0)
            },
            ...
            onClick: () => {
                setVitInitial(null), setVitOpen(!0)
            }
        }, React.default.createElement(Plus, {
            size: 15
        }), " Add vitamin or supplement")), React.default.createElement(RegimenSummaryCard, {
            icon: ClipboardList,
            iconBg: "#FBE7EE",
            iconColor: "#B23A6B",
            label: "Prescriptions",
        ...
        }, "Nothing added yet. Add prescriptions you manage yourself so they show up as one-tap options in Log It!, with next-due tracking."), rxItems.map(item => {
            let status = DS(item, todayKey);
            return React.default.createElement("div", {
                key: item.id,
                className: "wt-preset-row"
            }, React.default.createElement("span", {
                className: "wt-preset-name"
            }, item.name, item.intervalDays > 0 ? ` · every ${item.intervalDays}d` : ""), QS(item) && React.default.createElement("span", {
                style: {
                    fontSize: 11,
                    fontWeight: 700,
                    color: bS,
            ...
            }, QS(item)), status.due && React.default.createElement("input", {
                type: "date",
                className: "wt-treatment-date-input",
                value: status.due,
                onChange: evt => onEditSupplementNextDue(item.id, evt.target.value),
                "aria-label": `Change next due date for ${item.name}`
            }), React.default.createElement("button", {
                className: "wt-icon-btn",
                onClick: () => {
                    B(item), L(!0)
            ...
            size: 15
        }), " Add prescription")), React.default.createElement(RegimenSummaryCard, {
            icon: Users,
            iconBg: "#E0F2F1",
            iconColor: "#00695C",
            label: "Treatments",
            count: e.settings.treatments.length,
            preview: treatmentPreview,
            expanded: "treatments" === expandedList,
            onClick: () => setExpandedList("treatments" === expandedList ? null : "treatments")
        }), "treatments" === expandedList && React.default.createElement(PlanSheet, {
            title: "Treatments",
            onClose: () => setExpandedList(null)
        }, 0 === e.settings.treatments.length && React.default.createElement("p", {
            className: "wt-empty-note",
            style: {
                marginBottom: 10
            }
            ...
            onSave: (e2, t, n, r, a) => {
                vitInitial ? b(vitInitial.id, e2, t, n, r, a, "vitamin", null, null, null) : v(e2, t, n, r, a, "vitamin", null, null, null), setVitOpen(!1)
            }
        }), React.default.createElement(CO, {
            open: I,
            initial: R,
            ...
            partners: e.settings.partners,
            onClose: () => L(!1),
            onSave: (e2, t, n, r, a, ph, rf, partnerId) => {
                R ? b(R.id, e2, t, n, r, a, "rx", ph, rf, partnerId) : v(e2, t, n, r, a, "rx", ph, rf, partnerId), L(!1)
            }
            ...
            partners: e.settings.partners,
            onClose: () => F(!1),
            onSave: (e2, t, n, r, a, prov, partnerId) => {
                U ? E(U.id, e2, t, n, r, a, prov, partnerId) : x(e2, t, n, r, a, prov, partnerId), F(!1)
            }
        ...
        showAddPartner && React.default.createElement(PlanSheet, {
            ...
            onClose: () => {
                setShowAddPartner(!1), setEditingPartner(null)
            }
        }, React.default.createElement("label", {
            className: "wt-field"
            ...
            className: "wt-field"
        }, "Type", React.default.createElement("select", {
            value: partnerFormType,
            onChange: e => setPartnerFormType(e.target.value)
        }, React.default.createElement("option", { value: "treatment" }, "Treatment provider"), React.default.createElement("option", { value: "rx" }, "Pharmacy"))), React.default.createElement("label", {
            className: "wt-field"
        }, "Partner link (optional)", React.default.createElement("input", {
            type: "url",
            placeholder: "https://...",
            value: partnerFormLink,
            onChange: e => setPartnerFormLink(e.target.value)
        })), React.default.createElement("div", {
            style: {
                display: "flex",
                alignItems: "center",
        ...
        }, partnerFormLogo && React.default.createElement("img", {
            src: partnerFormLogo,
            alt: "",
            style: {
                width: 40,
        ...
        }, partnerFormLogo ? "Change Partner Logo" : "Upload Partner Logo (optional)", React.default.createElement("input", {
            type: "file",
            accept: "image/*",
            style: {
                display: "none"
            ...
            onChange: e => {
                ...
                t && readPartnerLogo(t, e => {
                    setPartnerFormLogo(e), setPartnerLogoError("")
                }, setPartnerLogoError)
            }
        })), partnerFormLogo && React.default.createElement("button", {
            className: "wt-btn-text wt-btn-text-danger",
            type: "button",
            onClick: () => setPartnerFormLogo(null)
        }, "Remove")), partnerLogoError && React.default.createElement("p", {
            style: {
                fontSize: 11.5,
                color: bS,
                marginTop: -8,
        ...
        }, partnerLogoError), React.default.createElement("button", {
            className: "wt-btn-primary",
            style: {
                width: "100%",
                marginTop: 8
            },
            disabled: 0 === partnerFormName.trim().length,
            onClick: () => {
                editingPartner ? onEditPartner(editingPartner.id, partnerFormName.trim(), partnerFormType, partnerFormLogo, partnerFormLink.trim() || null) : onAddPartner(partnerFormName.trim(), partnerFormType, partnerFormLogo, partnerFormLink.trim() || null), setShowAddPartner(!1), setEditingPartner(null)
            }
    ...
    function HO({
        ...
        return (0, React.useEffect)(() => {
        ...
        }, "© 2026 HydroPro Tracker Inc. All Rights Reserved.")), React.default.createElement("p", {
            style: {
                fontSize: 10,
                color: wD,
                margin: "18px 0 4px",
        ...
        }, "Demo / testing"), React.default.createElement("button", {
            className: "wt-btn-secondary",
            style: {
                width: "100%",
                fontSize: 12,
            ...
            onClick: () => {
                e.settings.demoSeed && e.settings.demoSeed.active ? onClearDemoData() : onLoadDemoData()
            }
        }, e.settings.demoSeed && e.settings.demoSeed.active ? "Clear demo data" : "Load demo data"), React.default.createElement(vO, {
            open: U,
    ...
    function ProfilePage({
        ...
        let n = e || {
            ...
            [r, a] = (0, React.useState)(n.fullName || ""),
            [o, i] = (0, React.useState)(n.email || ""),
            [l, u] = (0, React.useState)(n.phone || ""),
            [s, c] = (0, React.useState)(n.photoDataUri || ""),
            [f, d] = (0, React.useState)("");
        (0, React.useEffect)(() => {
            a(n.fullName || ""), i(n.email || ""), u(n.phone || ""), c(n.photoDataUri || "")
        }, [e]);
        let p = () => {
            t({
                fullName: r.trim(),
        ...
        return React.default.createElement("div", {
            ...
            className: "wt-profile-photo-wrap"
        }, s ? React.default.createElement("img", {
            src: s,
            alt: "",
            className: "wt-profile-photo"
        }) : React.default.createElement("div", {
            className: "wt-profile-photo wt-profile-photo-placeholder"
        ...
        }, s ? "Change Photo" : "Upload Photo", React.default.createElement("input", {
            type: "file",
            accept: "image/*",
            style: {
                display: "none"
            ...
            onChange: e => {
                let t = e.target.files && e.target.files[0];
                if (!t) return;
                if (t.size > 8388608) return void d("That image is too large. Please choose a smaller one.");
                let n = new FileReader;
                n.onload = () => {
                    let e = new Image;
                    e.onload = () => {
                        let t = 240,
                            n = e.width,
                            r = e.height,
                            a = Math.min(1, t / Math.max(n, r)),
                            o = Math.max(1, Math.round(n * a)),
                            i = Math.max(1, Math.round(r * a)),
                            l = document.createElement("canvas");
                        l.width = o, l.height = i;
                        let u = l.getContext("2d");
                        u.drawImage(e, 0, 0, o, i), c(l.toDataURL("image/jpeg", .85)), d("")
                    }, e.onerror = () => {
                        d("Couldn't read that image. Please try a different file.")
                    }, e.src = n.result
                }, n.onerror = () => {
                    d("Couldn't read that image. Please try a different file.")
                }, n.readAsDataURL(t)
            }
        })), s && React.default.createElement("button", {
            className: "wt-btn-text wt-btn-text-danger",
            style: {
                marginTop: 4
            },
            onClick: () => c("")
        }, "Remove Photo")), React.default.createElement("label", {
            className: "wt-field"
        }, "Full Name", React.default.createElement("input", {
            type: "text",
            placeholder: "e.g. Sarah M.",
            value: r,
            onChange: e => a(e.target.value)
            ...
            className: "wt-field"
        }, "Email Address", React.default.createElement("input", {
            type: "email",
            placeholder: "you@example.com",
            value: o,
            onChange: e => i(e.target.value)
            ...
            className: "wt-field"
        }, "Mobile Phone Number", React.default.createElement("input", {
            type: "tel",
        ...
        }, "This is separate from your account sign-in email — it's just for your profile."), React.default.createElement("button", {
            className: "wt-btn-primary",
            onClick: p
        }, "Save"), f && React.default.createElement("p", {
            style: {
                fontSize: 13,
                color: "var(--danger)",
                textAlign: "center",
    ...
    (0, ReactDOM.createRoot)(document.getElementById("root")).render(React.default.createElement(function() {
        ...
        let [t, n] = (0, React.useState)(null), [r, a] = (0, React.useState)("log"), [o, i] = (0, React.useState)(!1), [l, u] = (0, React.useState)(null), [s, c] = (0, React.useState)(null), [manualSheetOpen, setManualSheetOpen] = (0, React.useState)(!1), [f, d] = (0, React.useState)(null), [p, m] = (0, React.useState)(0), [h, g] = (0, React.useState)(!1), [y, v] = (0, React.useState)(0), [b, w] = (0, React.useState)(JS()), [x, E] = (0, React.useState)(null), [k, S] = (0, React.useState)(!1), [O, P] = (0, React.useState)(null), [C, j] = (0, React.useState)(!1), [N, T] = (0, React.useState)(!1), [A, M] = (0, React.useState)(null), [_, D] = (0, React.useState)(!1), [z, I] = (0, React.useState)(null), [L, R] = (0, React.useState)(!1), [B, $] = (0, React.useState)(null), [F, U] = (0, React.useState)(!1), W = (0, React.useRef)(null), H = (0, React.useRef)(null), q = (0, React.useRef)(null), V = (0, React.useRef)(null), G = (0, React.useRef)(!1), X = (0, React.useRef)(null), Y = (0, React.useRef)(null), [K, Q] = (0, React.useState)(null), [Z, J] = (0, React.useState)(null), [ee, te] = (0, React.useState)(typeof Notification < "u" ? Notification.permission : "unsupported"), [ne, re] = (0, React.useState)(!1), [ae, oe] = (0, React.useState)(null), ie = (0, React.useRef)(null), le = (0, React.useRef)(null), ue = (0, React.useRef)(null), se = (0, React.useRef)(!1), [logDate, setLogDate] = (0, React.useState)(() => new Date), [entryTargetDate, setEntryTargetDate] = (0, React.useState)(() => new Date), [rxSheetOpen, setRxSheetOpen] = (0, React.useState)(!1), [rxSheetInitial, setRxSheetInitial] = (0, React.useState)(null);
        ...
        function saveBackfill(e, r) {
            ...
            let rxItems = (r.rx || []).filter(e => e.name);
            rxItems.length > 0 && i.push({
                id: a(),
                type: "rx",
                label: `Took: ${rxItems.map(e=>e.qty?`${e.name} (${e.qty})`:e.name).join(", ")}`,
                items: rxItems,
                time: "12:00 PM",
                timeMinutes: 720,
                backfilled: !0,
            ...
            n(t => ({
                ...
                settings: {
                    ...t.settings,
                    rx: rxItems.length > 0 ? KS(t.settings.rx, [], rxItems) : t.settings.rx,
                    supplements: l.length > 0 ? KS(t.settings.supplements, [], l) : t.settings.supplements
                }
        ...
        return React.default.createElement("div", {
        ...
        }, "HydroPro Tracker"))), React.default.createElement("button", {
        ...
        }, t.settings.profile && t.settings.profile.photoDataUri ? React.default.createElement("img", {
            src: t.settings.profile.photoDataUri,
            alt: "",
            className: "wt-topbanner-profile-photo"
        }) : React.default.createElement(User, {
            size: 20,
            color: "var(--muted-dark)"
        }))), React.default.createElement("svg", {
            ...
            className: "wt-header"
        }, "profile" === r && React.default.createElement("button", {
            className: "wt-icon-btn",
            style: {
                marginRight: 4
            },
        ...
        }, "today" === r ? "TODAY'S INTAKES" : "rx" === r ? "RX FOR:" : "reports" === r ? "CURRENT STATS FOR:" : "setup" === r ? "My Plan for:" : "settings" === r ? "Settings" : "profile" === r ? "Profile" : "Tracking for:"), "settings" !== r && "profile" !== r && React.default.createElement(React.default.Fragment, null, " ", function(e) {
            return e.toLocaleDateString(void 0, {
                weekday: "long",
                month: "short",
                day: "numeric"
            ...
            onOpenSupplementSheet: () => {
                setEntryTargetDate(logDate), setRxSheetInitial(null), setRxSheetOpen(!0)
            },
            onOpenNewSupplementSheet: () => {
            ...
            onEditLogEntry: function(e) {
                setEntryTargetDate(new Date), PS(e) ? ($(e), R(!0)) : jS(e) ? (E(e), v(e.value), w(eO(e.timeMinutes)), g(!0)) : NS(e) ? (P(e), S(!0)) : RXTypeS(e) ? (setRxSheetInitial(e), setRxSheetOpen(!0)) : zS(e) ? (I({
                    ...e,
            ...
            onOpenSupplementSheet: () => {
                setEntryTargetDate(new Date), setRxSheetInitial(null), setRxSheetOpen(!0)
            },
            onOpenNewSupplementSheet: () => {
            ...
            data: t,
            todayKey: Ee,
            onEditSupplementNextDue: function(e, r) {
                n(t => ({
                    ...t,
                    settings: {
                        ...t.settings,
                        rx: t.settings.rx.map(t => t.id === e ? {
                            ...t,
                            nextDueOverride: r
                        } : t)
                    }
            ...
            onAddSupplement: function(e, t, r, a, o, c, ph, rf, partnerId) {
                let isRxCat = "rx" === c,
                    i = {
                        id: `${Date.now()}-${Math.random().toString(36).slice(2,7)}`,
                        name: e,
                ...
                n(e2 => ({
                    ...
                    settings: {
                        ...e2.settings,
                        rx: isRxCat ? [...e2.settings.rx, i] : e2.settings.rx,
                        supplements: isRxCat ? e2.settings.supplements : [...e2.settings.supplements, i]
                    }
                }))
            },
            onEditSupplement: function(e, t, r, a, o, i, c, ph, rf, partnerId) {
                let isRxCat = "rx" === c,
                    updater = arr => arr.map(n => n.id === e ? {
                        ...n,
                        name: t,
                        intervalDays: r || 1,
                        trackInventory: !!a,
                ...
                n(n2 => ({
                    ...
                    settings: {
                        ...n2.settings,
                        rx: isRxCat ? updater(n2.settings.rx) : n2.settings.rx,
                        supplements: isRxCat ? n2.settings.supplements : updater(n2.settings.supplements)
                    }
                }))
            },
            onDeleteSupplement: function(e) {
                n(t => ({
                    ...
                    settings: {
                        ...t.settings,
                        rx: t.settings.rx.filter(t => t.id !== e),
                        supplements: t.settings.supplements.filter(t => t.id !== e)
                    }
                }))
            },
            onAddTreatment: function(e, t, r, a, o, prov, partnerId) {
                let i = {
                    id: `${Date.now()}-${Math.random().toString(36).slice(2,7)}`,
                    name: e,
                    intervalDays: t,
            ...
            onEditTreatment: function(e, t, r, a, o, i, prov, partnerId) {
                n(n => ({
                    ...n,
                    settings: {
                        ...n.settings,
            ...
            onAddPartner: function(e, t, r, a) {
                ...
                n(e2 => ({
                    ...
                    settings: {
                        ...e2.settings,
                        partners: [...e2.settings.partners, i]
                    }
                }))
            },
            onEditPartner: function(e, t, r, a, o) {
                n(n => ({
                    ...n,
                    settings: {
                        ...n.settings,
                        partners: n.settings.partners.map(n => n.id === e ? {
                            ...n,
                            name: t,
                            type: r,
                            logoDataUri: a || null,
            ...
            onDeletePartner: function(e) {
                n(t => ({
                    ...t,
                    settings: {
                        ...t.settings,
                        partners: t.settings.partners.filter(t => t.id !== e),
                        rx: t.settings.rx.map(t => t.partnerId === e ? { ...t, partnerId: null } : t),
                        treatments: t.settings.treatments.map(t => t.partnerId === e ? { ...t, partnerId: null } : t)
                    }
                }))
            }
            ...
            onLoadDemoData: () => {
                n(t => buildDemoSeed(t)), ce("Demo data loaded — 30 days across every tracker.")
            },
            onClearDemoData: () => {
                n(t => clearDemoSeed(t)), ce("Demo data cleared.")
            },
            onOpenFeedback: () => U(!0),
        ...
        })), "profile" === r && React.default.createElement(ProfilePage, {
            ...
            targetDateLabel: entryTargetDateLabel,
            title: "Supplements",
            emptyCopy: "Nothing set up yet. Add the vitamins and supplements you take regularly in My Plan, and they'll show up here to log with one tap.",
            addButtonLabel: "Add to Supplements",
            borderColor: "var(--supplements)",
            onClose: () => {
                S(!1), P(null)
            },
            ...
            onSubmit: function(e, t) {
                ...
                function i(e, t) {
                    return e.settings.rx.map(e => o.includes(e.name) ? {
                        ...e,
                        lastTakenDate: t,
                        nextDueOverride: null
                    } : e)
                ...
                if (rxSheetInitial) {
                    ...
                    return n(n => ({
                        ...
                        logs: {
                            ...n.logs,
                            [t]: (n.logs[t] || []).map(t => t.id === rxSheetInitial.id ? {
                                ...t,
                                items: e,
                                label: a,
                                time: tO(r),
                        ...
                        settings: {
                            ...n.settings,
                            rx: KS(i(n, t), rxSheetInitial.items || [], e)
                        }
            ...
            size: 18
        }), " My Day"), React.default.createElement("button", {
